 ...
 const { todo, todos, doneTodos, handleTodoDone, titleChange, descriptionChange, buttonClicked, deleteClicked, editClicked } = useTodoContext();
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(to bottom, #5678FD, #C9D4FF)",
        minHeight: "100vh",
      }}
    >
      <div className="grow shrink-0 w-[500px]">
        <div className="pt-5">
          ...
          <div className="text-xs text-white">Check if the list is not marked as done before you edit </div>
          Title:{" "}
          <input
            name="title"
            value={todo.title}
            onChange={titleChange}
            className="shadow pb-2 appearance-none border rounded w-full py-1 px-2 text-shadow-800 leading-tight focus:outline-none focus:shadow-outline"
            id="title"
            type="text"
          ...
          <input
            ...
            className="shadow pb-2 appearance-none border rounded w-full py-2 px-3 text-shadow-700 leading-tight focus:outline-none focus:shadow-outline"
            id="description"
            type="text"
          />
          <button
            onClick={buttonClicked}
            className="mt-3 shadow bg-green-500 hover:bg-red-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded"
            type="button"
          >
